...
public class AuthUserService(
	...
	private static string ActivateUserQuery() {
		return "UPDATE users SET active = 1 WHERE userID = @id AND active = 0;";
	}
	...
	private static string UserRegistrationQuery(DateTime dtm) {
		...
		return @$"
			INSERT INTO users (name, surname, username, country, email, password, phone, birthdate, gender, createdAt, profilePicID) 
			VALUES (@name, @surname, @username, @country, @email, @password, @phone, @birthdate, @gender, '{timestamp}', @profilePicID);
